...
# JTrader — Journal de Trading Professionnel PHASE 10

Journal de trading moderne (SMC-focused) inspiré de TraderSync, Edgewonk et TradeZella.

## Stack
HTML5 · Tailwind CSS · JavaScript ES6 Modules · Chart.js · SheetJS (xlsx) · jsPDF + autoTable · Firebase (Auth, Firestore) · GitHub Pages
...
- ✅ **Phase 8 — Profil** (nom, email, date d'inscription, édition nom + photo via URL)
- ✅ **Phase 9 — Paramètres** (Dark Mode, capital de départ, devise, fuseau horaire, objectif mensuel)
- ✅ **Phase 10 — Export** (PDF et Excel, depuis l'historique, respecte recherche/filtres/tri actifs)

##  Les 10 phases sont terminées !

JTrader couvre maintenant l'intégralité du cahier des charges initial : authentification,
dashboard, gestion des trades (SMC), historique filtrable, statistiques, graphiques,
calendrier, profil, paramètres et export. Deux écarts assumés par rapport au brief
d'origine, documentés au fil du projet :
- **Pas d'upload de screenshots** (Firebase Storage exige désormais le plan payant
  Blaze) — remplacé par un champ URL, gratuit.
- **Fuseau horaire** stocké dans les paramètres mais pas encore branché dans le
  formatage des dates affichées (qui utilisent le fuseau local de l'appareil).
